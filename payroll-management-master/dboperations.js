...
function findAndUpdate(id, fullname, dob, bio, address, jopTitle, experience, totalProject) {
  return new Promise(function(resolve, reject) {
    employee.updateOne(
      { _id: id },
      {
      ...
      function(err, result) {
        if (err) {
          ...
          if (result.nModified === 0) {
            reject(new Error("Failed to update user profile")); // Reject the promise if no document was modified
          } else {
            resolve(result); // Resolve the promise with the updated document
          }
        }
      }
    );
